...
#         la BD de MongoDB.
#           docker run -t -i -e DB_URI=<uri_a_bd> <ImageID>
#
#       2)
#         Cambiar el puerto que usará docker:
#           docker run -t -e PORT=<puerto_deseado> -e DB_URI=<uri_a_bd> <ImageID>
# 
#       3)
#         Para mapear el puerto de docker al host se hace con:
#           docker run -t -i -e PORT=<puerto_en_docker> -e DB_URI=<uri_a_bd> -p <puerto_host>:<puerto_en_docker> <ImageID>